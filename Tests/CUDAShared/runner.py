...
exe = '../../executables/randshellsortCUDA'
sharedexe = '../../executables/randshellsortCUDANOSHARED'
sizes = []
s = 1024
while s <= 1024*1024*16:
	sizes.append(s)
	s = int(s*2)
algorithms = ['None', 'BitonicSort']
...
cudafiles = {alg: open(alg + 'CUDANOSHARED.res', 'w') for alg in algorithms}
...
for size in sizes:
	...
	for alg in algorithms:
		...
		proc = subprocess.Popen(['perf', 'stat', '-r', '10', '-e', 'task-clock,branch-misses,instructions,cache-misses', '-x,', sharedexe, 'Run',  alg, filename], stdout = subprocess.PIPE, stderr = subprocess.PIPE, universal_newlines = True)
		proc.wait()
		out, err = proc.communicate()
		line = str(size)
		#print(str(err))
