...
set title 'Time affected by cache misses'
set xlabel '$n$'
set ylabel 'time(ms)/$n \log^2 n$'
set y2label '\#cache misses/\#comparisons'
set logscale x
...
plot 	'OddEvenMergeSort.res' using 1:($2/($1*(log($1)/log(2))*(log($1)/log(2)))) title 'Time:OEMergeSort' with linespoints axes x1y1, \
		...
		'OddEvenMergeSort.res' using 1:($3/$6) title 'CacheMisses:OEMergeSort' with linespoints axes x1y2, \
		'OddEvenMergeSortBuffer.res' using 1:($3/$6) title 'CacheMisses:OEMergeSort\_DBuffer' with linespoints axes x1y2, \
		'OddEvenMergeSortSingle.res' using 1:($3/$6) title 'CacheMisses:OEMergeSort\_SBuffer' with linespoints axes x1y2 linecolor 0
