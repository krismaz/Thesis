...
plot  	'RShellSortPure.res' using 1:($4/($1*log($1)/log(2))) title 'RandShell' with linespoints, \
		...
		'ShakerSortSIMD.res' using 1:($4/($1*log($1)/log(2))) title 'ShakerSort\_SIMD' with linespoints linecolor 0, \
		'PrattSIMD.res' using 1:($4/($1*log($1)/log(2))) title 'Pratt\_SIMD' with linespoints, \
		'RShellSortPureSIMD.res' using 1:($4/($1*log($1)/log(2))) title 'RandShell\_SIMD' with linespoints, \
		'BitonicSortSIMD.res' using 1:($4/($1*log($1)/log(2))) title 'BitonicSort\_SIMD' with linespoints, \
		'OddEvenMergeSortSIMD.res' using 1:($4/($1*log($1)/log(2))) title 'OEMergeSort\_SIMD' with linespoints