...
simdexe = '../../executables/randshellsortSIMD'
sizes = []
s = 1024
...
simdfiles = {alg: open(alg + 'SIMD.res', 'w') for alg in algorithms}
...
for size in sizes:
	...
	for alg in algorithms:
		...
		proc = subprocess.Popen(['perf', 'stat', '-r', '10', '-e', 'task-clock,branch-misses,instructions,cache-misses', '-x,', simdexe, 'Run',  alg, filename], stdout = subprocess.PIPE, stderr = subprocess.PIPE, universal_newlines = True)
		proc.wait()
		out, err = proc.communicate()
		line = str(size)
		#print(str(err))
