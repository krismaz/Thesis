...
swapexe = '../../executables/randshellsortSWAP'
sizes = []
s = 1024
while s <= 1024*1024*16:
	sizes.append(s)
	s = int(s*2)
algorithms = ['None','RShellSortPure', 'AnnealingSortPure','RShellSortPure','OddEvenMergeSort' , 'ShakerSort','Pratt']
counters = ['branch-misses', 'comparisons']
...
swapfiles = {alg: open(alg + 'Swap.res', 'w') for alg in algorithms}
MMfiles = {alg: open(alg + 'MinMax.res', 'w') for alg in algorithms}
...
for size in sizes:
	...
	for alg in algorithms:
		...
		proc = subprocess.Popen(['perf', 'stat', '-r', '10', '-e', 'task-clock,branch-misses,instructions,cache-misses', '-x,', swapexe, 'Run',  alg, filename], stdout = subprocess.PIPE, stderr = subprocess.PIPE, universal_newlines = True)
		proc.wait()
		out, err = proc.communicate()
		line = str(size)
		#print(str(err))
